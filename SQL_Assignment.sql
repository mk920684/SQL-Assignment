...
select ad_id, case when action='Clicked' 
then count(ad_id)/count(ad_id)+case when action='Viewed' then count(ad_id) end from Ads group by ad_id;
...
select
ad_id,case when action='Clicked' then 1 else 0 end ctr
from Ads
group by ad_id;

select  case when action='Clicked' 
then 1 else 0 end from Ads;
